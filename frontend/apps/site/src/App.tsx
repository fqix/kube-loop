...
const repo = "https://github.com/fqix/kube-loop";
...
function GetStarted({ t, locale }: { t: typeof copy.en; locale: Locale }) { const steps = locale === "zh" ? [["01", "安装客户端", "安装适用于 macOS、Windows 或 Linux 的 桌面客户端。"], ["02", "添加 Server", "填写管理员提供的 KubeLoop Server HTTP 或 HTTPS 地址并完成能力发现。"], ["03", "登录", "在系统浏览器完成登录。无需邀请或权限分配；如有需要，再选择 Namespace 上下文。"], ["04", "以 TUN 模式连接", "TUN 是默认模式，首次使用时需批准本地 Helper；不希望使用特权网络时可切换到 SOCKS。"]] : [["01", "Install a client", "Install the Desktop client for macOS, Windows, or Linux."], ["02", "Add a Server", "Enter the HTTP or HTTPS KubeLoop Server URL supplied by your administrator and run discovery."], ["03", "Sign in", "Complete sign-in in the system browser. No invitation or permission assignment is required; choose a namespace context only when needed."], ["04", "Connect with TUN", "TUN is the default; approve the local Helper on first use. Switch to SOCKS when privileged networking is not desired."]]; return <div className="section"><PageIntro eyebrow={locale === "zh" ? "开始" : "START"} title={t.quick} lead={t.quickLead} /><ServerInstallBlock locale={locale} /><div className="mt-6"><InstallBlock locale={locale} /></div><div className="mt-10 grid gap-4 md:grid-cols-2">{steps.map(([n,title,body]) => <Card key={n}><CardHeader><Badge className="w-fit">{n}</Badge><CardTitle className="mt-3">{title}</CardTitle><CardDescription>{body}</CardDescription></CardHeader></Card>)}</div><Notice locale={locale} /></div>; }
