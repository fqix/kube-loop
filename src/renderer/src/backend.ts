...
/**
 * Bound methods exposed by the Go backend (internal/app.App). The Electron
 * preload forwards `call(name, ...args)` to the sidecar over JSON-RPC; the
 * method names and positional arguments are unchanged from the previous
 * in-process binding layer.
 */
...
export interface KubeLoopBridge {
...
declare global {
  interface Window {
    kubeloop?: KubeLoopBridge;
  }
}

export function bridge(): KubeLoopBridge {
  const value = window.kubeloop;
  if (!value) {
    throw new Error(
      "KubeLoop backend is unavailable. Run this interface inside the desktop app (`npm run dev`).",
    );
  }
  return value;
}

// Every BackendApi member becomes `bridge().call(name, ...args)`.
const apiProxy = new Proxy({} as BackendApi, {
  get(_target, name) {
    if (typeof name !== "string") return undefined;
    return (...args: unknown[]) => bridge().call(name, ...args);
  },
});
...
function subscribe<T>(name: string, callback: (payload: T) => void): () => void {
  if (!window.kubeloop) return () => undefined;
  return window.kubeloop.on(name, callback as (payload: unknown) => void);
}
...
export const backend = {
  ...
  onSession: (callback: (state: SessionState) => void) => {
    return subscribe("session:state", callback);
  },
  onUpdate: (callback: (state: UpdateInfo) => void) => {
    return subscribe("update:state", callback);
  },
  onTransfer: (callback: (task: FileTransferTask) => void) => {
    return subscribe("transfer:updated", callback);
  },
  onServerExec: (callback: (event: ServerExecEvent) => void) => {
    return subscribe("server-exec:event", callback);
  },
  onServerFileTransfer: (callback: (task: ServerFileTransferTask) => void) => {
    return subscribe("server-file-transfer:event", callback);
  },
};
