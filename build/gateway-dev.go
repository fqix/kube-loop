...
func main() {
	...
	fmt.Printf(
		"==> Desktop development images ready: Control Plane %s, Gateway %s, Operator %s\n",
		controlPlaneImage, gatewayImage, operatorImage,
	)
}

// buildDevelopmentSingBox uses the same staging command as a packaged
// desktop build. Keeping the binary under build/bin lets the development application
...
func buildLinuxBinary(root, output, packagePath string) error {
	...
	// Earlier tooling could recreate build/bin with owner-only permissions. Minikube copies
